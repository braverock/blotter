...
Some variables are annualized and hence need the total number of business days in a given market and within a given year (typically a factor of 252 days, in 
the US markets, or of 250 days), we denote it $T_{m}$. Below we provide a schematic view of these and other quantities involved are defined below: 
...
To compare the *optimal holding trajectories*:
```{r}

# TODO: The risk-averse case curvature in incorrect. See the explanation I argued here https://github.com/braverock/blotter/pull/99

# plot(ac_averse$Optimal.Trajs[, 'Opt.Hold'], type = 'b', main = 'Order Execution (risk profiles)', xlab = "Trading period", ylab = "Optimal holding", col = 'blue')
...
# TODO: as a consequence (as argued https://github.com/braverock/blotter/pull/99), the effiecient frontier is incorrect 

# lambdas <- seq(-5e-07, 5e-06, 1e-07)
# opt.pts <- matrix(NA, nrow = length(lambdas), ncol = 2)
# for (l in 1:length(lambdas)) {
#   opt.pts[l, ] <- acOptTxns(Securities = Securities, mu = 0, sigma = sigma, epsilon = epsilon, gamma = gamma, eta = eta, lambda = lambdas[l])$Optimal.Comb
#   e <- opt.pts[, 1]
#   v <- opt.pts[, 2]
# }
# plot(v, e, type = 'l', lwd = 1.5, main = "Efficient Trading Frontier", xlab = "Variance, V(x)", ylab = "Expected loss, E(x)")
